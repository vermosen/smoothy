#include "tests/meta.h"
#include "../definitions.h"

#include <cstdint>

#include <smoothy/definitions.h>
#include <smoothy/meta/patterns/curiousmixin.h>

#include <meta/cpuclock.h>

namespace smoothy {
namespace testsuite {

  boost::unit_test_framework::test_suite* meta_test::suite() {
    boost::unit_test_framework::test_suite* suite = BOOST_TEST_SUITE("meta");
    suite->add(BOOST_TEST_CASE(&meta_test::curiousmixins), 0, MAX_TIME_SEC);
    suite->add(BOOST_TEST_CASE(&meta_test::clockmeasurement), 0, MAX_TIME_SEC);
    return suite;
  }

  template <typename Base>
  struct elem1 {
    elem1() : m_i(0) {}
    elem1(int i) : m_i(i) {}
    int m_i;
  };

  template <typename Base>
  struct elem2 {
    elem2() : m_j(0.0) {}
    elem2(double j) : m_j(j) {}
    double m_j;
  };

  struct mixin : public smoothy::meta::curiousMixin<mixin, elem1, elem2> {
    mixin() : smoothy::meta::curiousMixin<mixin, elem1, elem2>() {}
    mixin(int i, double d) : smoothy::meta::curiousMixin<mixin, elem1, elem2>(i, d) {}
  };

  void meta_test::curiousmixins() {
    mixin m1;           // default mixin ctor
    mixin m2(1, 2.0);   // other ctor
  }

  inline std::uint64_t fibo(unsigned t) {

    if (t < 2) {
      return 1;
    }
    else if (t == 2) {
      return 2;
    }
    else {
      std::uint64_t t1, t2;
      t1 = 1; t2 = 1;
      for (std::size_t i = 2; i < t; i++) {
        auto tmp = t1;
        t1 = t2;
        t2 += tmp;
      }

      return t2;
    }
  }

  void meta_test::clockmeasurement() {

    using namespace ::meta::clock;
    auto start = cpu<position::pre>::now();

    auto res = fibo(100);

    auto diff = cpu<position::post>::now() - start;

    std::stringstream ss; ss 
      << "result: " << res << '\n'
      << "duration: " << diff << " ticks" << std::endl;

    BOOST_TEST_MESSAGE(ss.str());
  }
}}
